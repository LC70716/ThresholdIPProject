...
function [w_matrix,k_matrix,psnr_vals,ssim_vals,mssim_vals] = batch_best_parameters(ref_images,cor_images)
    w_matrix = zeros(4,10); % 3 algorithms, so first index has length 3. 10 different window sizes,so second index has length 10.
    k_matrix = zeros(4,40);
    psnr_vals = zeros(4,size(ref_images));
    ssim_vals = zeros(4,size(ref_images));
    mssim_vals = zeros(4,size(ref_images));
