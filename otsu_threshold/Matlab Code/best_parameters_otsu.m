function [w_array,k_array,psnr_vals,ssim_vals,mssim_vals] = best_parameters_otsu(corrupt, gr_truth)
    j = 0; % window index
    ...
    for window = 5:10:105 % window size for loop
        ...
        for bias_k = 0:0.025:1 % bias for loop
            % CALLING OTSU THRESHOLD FUNCTION
            thresholded = otsu_threshold(corrupt, window, bias_k);
